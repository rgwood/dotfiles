# edit_command_buffer from the fish standard library, customized to use VS code
# necessary b/c Fish doesn't support arguments in $VISUAL well,
# it will fail with "Unknown command 'code --wait --new-window'"
# Didn't bother ripping out most existing code -Reilly
function edit_command_buffer_in_vs_code --description 'Edit the command buffer in an external editor'
    set -l f (mktemp)
    if set -q f[1]
        mv $f $f.fish
        set f $f.fish
    ...
    if set -q VISUAL
        ...
        # Reilly: this is the only change
        code --wait --new-window $f
